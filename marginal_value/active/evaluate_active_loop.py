...
def run_active_loop_eval(config: dict[str, Any], *, smoke: bool = False) -> dict[str, Any]:
    ...
    embedding_result = load_embedding_lookup(
        ...
        imagebind_imu_config=config.get("imagebind_imu") if isinstance(config.get("imagebind_imu"), Mapping) else None,
    )
    embedding_lookup = embedding_result.embeddings
    novelty_correlations = _novelty_rank_correlations(
        episodes=episodes,
    ...
    report = {
        ...
        "novelty_rank_correlations": novelty_correlations,
...
def validate_active_loop_eval_config(config: Mapping[str, Any]) -> None:
    ...
    unknown = set(policies) - supported
    if unknown:
...
def _novelty_rank_correlations(
    *,
    ...
    if "window_mean_std_pool" not in representations:
        return {}
    comparisons = [
        rep
        for rep in representations
        if rep.startswith("imu2clip") or rep == "imagebind_imu"
    ]
    if not comparisons:
        return {}
    rows: dict[str, list[float]] = {rep: [] for rep in comparisons}
    for episode in episodes:
        support_ids = list(episode.support_clip_ids)
        candidate_ids = list(episode.candidate_clip_ids)
        if not support_ids or not candidate_ids:
            continue
        _, baseline_novelty = old_novelty_only_order(
            _stack(embeddings["window_mean_std_pool"], support_ids),
            _stack(embeddings["window_mean_std_pool"], candidate_ids),
            k=min(max(1, int(old_novelty_k)), len(support_ids)),
        )
        for representation in comparisons:
            _, other_novelty = old_novelty_only_order(
                _stack(embeddings[representation], support_ids),
                _stack(embeddings[representation], candidate_ids),
                k=min(max(1, int(old_novelty_k)), len(support_ids)),
            )
            rows[representation].append(_rank_correlation(baseline_novelty, other_novelty))
    return {
        representation: {
            "episode_count": float(len(values)),
            "mean_spearman": float(np.mean(values)) if values else 0.0,
        }
...
def _rank_correlation(left: np.ndarray, right: np.ndarray) -> float:
    ...
    if left_values.shape != right_values.shape or left_values.size < 2:
        return 0.0
    left_rank = _rankdata(left_values)
    right_rank = _rankdata(right_values)
