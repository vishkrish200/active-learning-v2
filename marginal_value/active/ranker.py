...
from marginal_value.active.embedding_cache import embedding_cache_dir_from_config, load_embedding_lookup
            ...
            rows.append(
                {
                    ...
                    "target": float(label.get(target, 0.0)),
                }
            )
            feature_dicts.append(feature)
            labels.append(float(label.get(target, 0.0)))
        log_progress(
            "active_ranker",
...
def validate_active_ranker_config(config: Mapping[str, Any]) -> None:
    ...
    unsupported = set(str(rep) for rep in representations) - set(DEFAULT_REPRESENTATIONS)
    if unsupported:
        raise ValueError(f"Unsupported active-ranker representations: {sorted(unsupported)}")
    if str(ranker.get("target", "balanced_gain")) not in {"balanced_gain", "balanced_relative_gain", "balanced_gain_after_greedy_prefix"}:
        raise ValueError("ranker.target must be a supported marginal-gain label.")
