...
from marginal_value.active_benchmark.downstream_smoke import linear_reconstruction_error
from marginal_value.active_benchmark.downstream_supervised_smoke import (
    _labels_by_clip_id,
    supervised_classification_metrics,
)
...
def build_downstream_coverage_supervised_rows(
    ...
    *,
    ...
    downstream_models: Sequence[str] = ("nearest_centroid",),
            ...
            for budget_k in budgets:
                ...
                for model_name in downstream_models:
                    for representation in downstream_representations:
                        baseline = supervised_classification_metrics(
                            clips_by_id,
                        ...
                        after = supervised_classification_metrics(
                            clips_by_id,
                            labels_by_id=labels_by_id,
    ...
    *,
    ...
    return {
        "input": {
            ...
            "downstream_models": [str(model) for model in downstream_models],
            "label_source": str(label_source),
            "label_representation": str(label_representation),
...
def build_downstream_coverage_utility_rows(
    result: CoverageRunResult,
    clips: Sequence[BenchmarkClip],
    *,
    downstream_representations: Sequence[str],
                    ...
                    rows.append(
                        {
                            ...
                            "relative_reconstruction_gain": (
                                float(absolute_gain / baseline_error) if baseline_error > 1.0e-12 else 0.0
                            ),
                        }
                    )
    return rows
...
def build_downstream_coverage_utility_report(
    result: CoverageRunResult,
    clips: Sequence[BenchmarkClip],
    *,
    downstream_representations: Sequence[str],
    ...
    summary = summarize_downstream_coverage_utility(rows)
    return {
        "input": {
            "episode_count": int(len(result.episodes)),
            "policy_count": int(len(result.policies)),
        ...
        "summary": summary,
        "decision": _utility_decision(summary, top_policy=str(top_policy), baseline_policy=str(baseline_policy)),
        "rows": rows,
    }


def summarize_downstream_coverage_utility(rows: Sequence[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "policy_budget_means": _utility_policy_budget_means(rows_list),
        "policy_final_means": _utility_policy_means(final_rows),
    }


def write_downstream_coverage_utility_reports(report: dict[str, object], output_dir: str | Path) -> dict[str, Path]:
    root = Path(output_dir)
    root.mkdir(parents=True, exist_ok=True)
    json_path = root / "downstream_coverage_utility_report.json"
    markdown_path = root / "downstream_coverage_utility_report.md"
    json_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    markdown_path.write_text(_utility_markdown_report(report), encoding="utf-8")
...
def summarize_downstream_coverage_supervised(rows: Sequence[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "policy_model_final_means": _policy_model_means(final_rows),
    }
...
def _policy_model_means(rows: Sequence[dict[str, object]]) -> dict[str, dict[str, dict[str, float | int]]]:
    by_policy_model: dict[str, dict[str, list[dict[str, object]]]] = defaultdict(lambda: defaultdict(list))
    for row in rows:
        by_policy_model[str(row["policy_id"])][str(row.get("model_name", "nearest_centroid"))].append(dict(row))
    return {
        policy_id: {model_name: _mean_row_values(model_rows) for model_name, model_rows in sorted(model_map.items())}
        for policy_id, model_map in sorted(by_policy_model.items())
    }


def _utility_policy_budget_means(rows: Sequence[dict[str, object]]) -> dict[str, dict[str, dict[str, float | int]]]:
    by_policy_budget: dict[str, dict[int, list[dict[str, object]]]] = defaultdict(lambda: defaultdict(list))
    for row in rows:
        by_policy_budget[str(row["policy_id"])][int(row["budget_k"])].append(dict(row))
    return {
        policy_id: {
            str(budget_k): _utility_mean_row_values(budget_rows)
            for budget_k, budget_rows in sorted(budget_map.items())
        }
        for policy_id, budget_map in sorted(by_policy_budget.items())
    }


def _utility_policy_means(rows: Sequence[dict[str, object]]) -> dict[str, dict[str, float | int]]:
    by_policy: dict[str, list[dict[str, object]]] = defaultdict(list)
    for row in rows:
        by_policy[str(row["policy_id"])].append(dict(row))
    return {policy_id: _utility_mean_row_values(policy_rows) for policy_id, policy_rows in sorted(by_policy.items())}
...
def _utility_mean_row_values(rows: Sequence[dict[str, object]]) -> dict[str, float | int]:
    return {
        "row_count": int(len(rows)),
...
def _decision(summary: dict[str, object], *, top_policy: str, baseline_policy: str) -> dict[str, object]:
    ...
    read = (
        "source-family pseudo-label bridge downstream benchmark ran with predeclared frozen model heads; keep this as an identifiability gate and do "
    ...
    return {
        ...
        "top_policy": str(top_policy),
        "baseline_policy": str(baseline_policy),
        "models": _model_names_from_summary(summary),
...
def _utility_decision(summary: dict[str, object], *, top_policy: str, baseline_policy: str) -> dict[str, object]:
    policy_means = summary.get("policy_final_means", {})
    if not isinstance(policy_means, dict):
        policy_means = {}
    top = policy_means.get(top_policy, {})
    baseline = policy_means.get(baseline_policy, {})
    top_gain = _metric_value(top, "mean_relative_reconstruction_gain")
    baseline_gain = _metric_value(baseline, "mean_relative_reconstruction_gain")
    top_after_error = _metric_value(top, "mean_after_reconstruction_error")
    baseline_after_error = _metric_value(baseline, "mean_after_reconstruction_error")
    gain_delta = top_gain - baseline_gain if top_gain is not None and baseline_gain is not None else None
    after_error_delta = (
        baseline_after_error - top_after_error
        if top_after_error is not None and baseline_after_error is not None
        else None
    )
    return {
        "downstream_training": "heldout_utility_probe_only",
        "read": (
            "hidden-target reconstruction utility probe ran on frozen features; this can justify one bounded "
            "downstream experiment, but it is not challenge-label training proof."
        ),
        "top_policy": str(top_policy),
        "baseline_policy": str(baseline_policy),
        "top_mean_relative_reconstruction_gain": top_gain,
        "baseline_mean_relative_reconstruction_gain": baseline_gain,
        "relative_reconstruction_gain_delta_vs_baseline": gain_delta,
        "top_mean_after_reconstruction_error": top_after_error,
        "baseline_mean_after_reconstruction_error": baseline_after_error,
        "after_reconstruction_error_delta_vs_baseline": after_error_delta,
        "next_steps": [
            "Use this as the smallest held-out utility gate after the source-family bridge proxy.",
            "Require the TS2Vec k-center policy to beat quality-stratified random before scaling.",
            "Keep TS2Vec retraining and large neural downstream training held.",
        ],
    }
...
def _model_names_from_summary(summary: dict[str, object]) -> list[str]:
    policy_model = summary.get("policy_model_final_means", {})
    if not isinstance(policy_model, dict):
        return []
    names = set()
    for model_map in policy_model.values():
        if isinstance(model_map, dict):
            names.update(str(model_name) for model_name in model_map)
    return sorted(names)
...
def _markdown_report(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"- downstream model heads: `{', '.join(str(item) for item in input_block.get('downstream_models', []))}`",
        "",
        "## Budget Means",
        "",
...
def _utility_markdown_report(report: dict[str, object]) -> str:
    input_block = report.get("input", {})
    summary = report.get("summary", {})
    decision = report.get("decision", {})
    policy_budget_means = summary.get("policy_budget_means", {}) if isinstance(summary, dict) else {}
    lines = [
        "# Downstream Coverage Utility Probe",
        "",
        "This is a hidden-target linear reconstruction utility probe on frozen features, not challenge-label training proof.",
        "",
        "## Decision",
        "",
        f"- downstream training: `{decision.get('downstream_training')}`",
        ...
        f"- top policy: `{decision.get('top_policy')}`",
        f"- baseline policy: `{decision.get('baseline_policy')}`",
        f"- relative reconstruction gain delta vs baseline: `{_fmt(decision.get('relative_reconstruction_gain_delta_vs_baseline'))}`",
        f"- after-error delta vs baseline: `{_fmt(decision.get('after_reconstruction_error_delta_vs_baseline'))}`",
        "",
        "## Run",
        "",
        f"- episodes: `{input_block.get('episode_count')}`",
        f"- budgets: `{', '.join(str(item) for item in input_block.get('budgets', []))}`",
        f"- downstream representations: `{', '.join(str(item) for item in input_block.get('downstream_representations', []))}`",
        f"- max components: `{input_block.get('max_components')}`",
        "",
        "## Budget Means",
        "",
        "| policy | budget | relative gain | absolute gain | after error | rows |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    if isinstance(policy_budget_means, dict):
        for policy_id, budget_map in sorted(policy_budget_means.items()):
            ...
            for budget_k, values in sorted(budget_map.items(), key=lambda item: int(item[0])):
                if not isinstance(values, dict):
                    continue
                lines.append(
                    "| {policy} | {budget} | {rel} | {abs_gain} | {after} | {rows} |".format(
                        policy=policy_id,
                        budget=budget_k,
                        rel=_fmt(values.get("mean_relative_reconstruction_gain")),
                        abs_gain=_fmt(values.get("mean_absolute_reconstruction_gain")),
                        after=_fmt(values.get("mean_after_reconstruction_error")),
                        rows=int(values.get("row_count", 0)),
                    )
                )
    lines.extend(
        [
            ...
            "## Caveat",
            "",
            "- Budget `0` is the support-only baseline. Later budgets add selected candidate clips to support before measuring hidden-target reconstruction error.",
            "- This checks whether acquisition reduces held-out representation error; it is not task-label supervision or TS2Vec retraining.",
        ]
    )
    return "\n".join(lines) + "\n"
