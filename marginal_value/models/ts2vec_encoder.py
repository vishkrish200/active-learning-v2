        ...
        *,
        ...
        class _TS2VecEncoder(nn.Module):
            def __init__(self) -> None:
                ...
                self.hidden_projection = nn.Identity() if input_dims == hidden_dims else nn.Linear(input_dims, hidden_dims)
                self.blocks = nn.ModuleList(_CausalConvBlock(hidden_dims, dilation=2**idx) for idx in range(n_layers))
