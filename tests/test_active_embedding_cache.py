...
class ActiveEmbeddingCacheTests(unittest.TestCase):
    def test_imu2clip_representation_uses_configured_inference_wrapper(self):
        with TemporaryDirectory() as tmp:
            ...
            class FakeIMU2CLIPInference:
                def __init__(self, **kwargs):
                    self.kwargs = kwargs

                def encode_clip(self, clip):
                    vector = np.zeros(512, dtype=np.float32)
                    vector[0] = float(len(clip))
                    vector[1] = 1.0
                    return vector / np.linalg.norm(vector)

                def encode_clip_multiscale(self, clip):
                    vector = np.zeros(1536, dtype=np.float32)
                    vector[0] = float(len(clip))
                    vector[1] = 1.0
                    return vector / np.linalg.norm(vector)

            with patch("marginal_value.active.embedding_cache.IMU2CLIPInference", FakeIMU2CLIPInference):
                result = load_embedding_lookup(
                    [clip_a, clip_b],
                    representations=["imu2clip", "imu2clip_multiscale"],
                    sample_rate=30.0,
                    raw_shape_max_samples=None,
                    cache_dir=None,
                    component="test_embedding_cache",
                    mode="smoke",
                    imu2clip_config={"checkpoint_path": "unit-checkpoint.pt", "device": "cpu"},
                )

        self.assertEqual(result.cache_status, "disabled")
        self.assertEqual(result.embeddings["imu2clip"]["clip-a"].shape, (512,))
        self.assertEqual(result.embeddings["imu2clip_multiscale"]["clip-a"].shape, (1536,))
        np.testing.assert_allclose(np.linalg.norm(result.embeddings["imu2clip"]["clip-b"]), 1.0, atol=1.0e-6)

    def test_imu2clip_representation_requires_checkpoint_config(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_imagebind_imu_representation_uses_configured_inference_wrapper(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            clip_a = _write_clip(root, "clip-a", center=np.asarray([1.0, 0.0, 0.0, 0.0], dtype=float))
            clip_b = _write_clip(root, "clip-b", center=np.asarray([0.0, 1.0, 0.0, 0.0], dtype=float))

            class FakeImageBindIMUInference:
                def __init__(self, **kwargs):
                    self.kwargs = kwargs
            ...
            with patch("marginal_value.active.embedding_cache.ImageBindIMUInference", FakeImageBindIMUInference):
                result = load_embedding_lookup(
                    [clip_a, clip_b],
                    representations=["imagebind_imu"],
                    sample_rate=30.0,
                    raw_shape_max_samples=None,
                    cache_dir=None,
                    component="test_embedding_cache",
                    mode="smoke",
                    imagebind_imu_config={"device": "cpu", "pretrained": False},
                )

        self.assertEqual(result.cache_status, "disabled")
        self.assertEqual(result.embeddings["imagebind_imu"]["clip-a"].shape, (1024,))
        np.testing.assert_allclose(np.linalg.norm(result.embeddings["imagebind_imu"]["clip-b"]), 1.0, atol=1.0e-6)
