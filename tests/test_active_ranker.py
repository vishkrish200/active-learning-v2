...
class ActiveRankerTests(unittest.TestCase):
    ...
    def test_modal_active_ranker_train_entrypoint_uses_remote_gpu_job(self):
        ...
        self.assertEqual(config["labels"]["path"], "/artifacts/active/labels/scale_pretrain/active_label_gain_full.csv")
        self.assertEqual(config["ranker"]["target"], "balanced_gain")
...
def _memory_fixture():
    clips = []
    embeddings = {"window_mean_std_pool": {}, "window_shape_stats": {}}
    labels = []
    episode_rows = []
    for episode_idx in range(3):
        ...
        labels.append({"episode_id": episode_id, "sample_id": known, "candidate_role": "known_like", "balanced_gain": 0.0})
        labels.append({"episode_id": episode_id, "sample_id": good, "candidate_role": "heldout_novel", "balanced_gain": 0.25})
...
def _write_ranker_fixture(root: Path) -> dict[str, str]:
    ...
    for episode_idx in range(3):
        ...
        label_rows.append({"episode_id": episode_id, "sample_id": ids["known"], "candidate_role": "known_like", "balanced_gain": 0.0})
        label_rows.append({"episode_id": episode_id, "sample_id": ids["good"], "candidate_role": "heldout_novel", "balanced_gain": 0.25})
